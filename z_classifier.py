...
parser = argparse.ArgumentParser(description='Generator and Encoder')
# RNG
parser.add_argument('--seed', type=int, help='RNG seed')

# Objective (which modules to train? what losses to minimize?)
parser.add_argument('--discrim_weight', type=float, default=0.0,
    help='Weighting of discrim loss')
parser.add_argument('--joint_discrim_weight', type=float, default=1.0,
    help='Train joint (BiGAN) discriminator (x, z) with this weight')
parser.add_argument('--encode', default=True, help='Learn encoder')
parser.add_argument('--encode_weight', type=float, default=0.0,
    help='Encoder weighting of actual encode loss')
parser.add_argument('--encode_normalize', default=True,
    help='Normalize encoder costs by distribution dimension')
parser.add_argument('--encode_gen_weight', type=float, default=1.0,
...
parser.add_argument('--no_decay_gain', default=True,
...
parser.add_argument('--gen_net_size', default=64, type=int,
...
parser.add_argument('--feat_net_size', default=64, type=int,
...
# Encoder
parser.add_argument('--encode_net', default='alexnet_group_padpool',
...
parser.add_argument('--raw_size', default=72, type=int,
...
parser.add_argument('--max_labels', default=1000, type=int,
...
parser.add_argument('--disp_interval', default=1, type=int,
    help='Compute and display test accuracy every disp_interval epochs')
parser.add_argument('--deploy_iters', type=int, default=1000,
...
parser.add_argument('--disp_samples', type=int, default=400,
    help='Max number of samples to display')
parser.add_argument('--megabatch_gb', type=float, default=0.5,
...
parser.add_argument('--classifier', default=True,
    help='Learn classifier from discrim (and encoder) feats')
parser.add_argument('--classifier_deploy', default=True,
...
parser.add_argument('--resume', default=100, type=int,
...
if __name__ == '__main__':
    ...
    images, labels = dataset.train_provider.get_batch()
    X = Output(input_transform(images[0]), (args.batch_size, nc, args.crop_size, args.crop_size))
    with nostdout():
    ...
    evaluate = False # option to just evaluate model, and not train
    workers = 4
    print_freq = 10
    start_epoch = 0
    epochs = 1
    ...
    # Data loading (only for Validation Set)
    valdir = '/home/choidami/ml/bigan/data/imagenet72/val/'
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i in range(iters_in_epoch):
            ...
            with nostdout():
                hs = sample_z_from_x(X)[0].value.eval()
            input_h = torch.from_numpy(hs).cuda()
            labels = torch.from_numpy(np.int64(labels)).cuda()
            input_h_val, labels_val = Variable(input_h), Variable(labels)
            
            # measure data loading time
            data_time.update(time.time() - end)
            
            # compute output 
            output = h_classifier(input_h_val)
            loss = criterion(output, labels_val)
            
            # measure accuracy and record loss
            prec1, prec5 = accuracy(output.data, labels, topk=(1, 5))
            losses.update(loss.data[0], input_h.size(0))
            top1.update(prec1[0], input_h.size(0))
            top5.update(prec5[0], input_h.size(0))
            ...
            if i % print_freq == 0:
                print('Epoch: [{0}][{1}/{2}]\t'
                      ...
                      'Prec@1 {top1.val:.3f} ({top1.avg:.3f})\t'
                      'Prec@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                       epoch, i, iters_in_epoch, batch_time=batch_time,
        ...
        prec1 = validate(val_loader, h_classifier, criterion, print_freq, args)
...
#        X = Output(input_transform(images[0]), (args.batch_size, nc, args.crop_size, args.crop_size))
#        hs = sample_z_from_x(X)[0].value.eval()
#        hs, labels = Variable(torch.from_numpy(hs)), Variable(torch.from_numpy(np.int64(labels)))
#        
#        optimizer.zero_grad()
#        
...
#            running_loss = 0.0
#            
#    pytorch_util.validate(val_loader, h_classifier, criterion)
